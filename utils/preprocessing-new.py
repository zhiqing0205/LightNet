...
def get_edges_of_pc(organized_pc):
    # 使用更高效的方式获取边缘点，避免重复reshape和concatenate
    edges_mask = np.zeros(organized_pc.shape[:2], dtype=bool)
    edges_mask[:10, :] = True
    edges_mask[-10:, :] = True
    edges_mask[:, :10] = True
    edges_mask[:, -10:] = True
    return organized_pc[edges_mask]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_workers', type=int, default=2, help='Number of workers for parallel processing')
    ...
    print(type(paths))
    paths = list(paths)
    print(f"Found {len(paths)} tiff files in {root_path}")


    # with ProcessPoolExecutor() as executor:
    #     list(tqdm(executor.map(preprocess_pc, paths), total=len(paths)))
