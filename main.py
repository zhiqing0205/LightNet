import argparse
from lightnet_runner import LightNet
...
def run_3d_ads(args):
    ...
    for cls in classes:
        model = LightNet(args)
        image_rocaucs, pixel_rocaucs, au_pros = model.fit(cls)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method_name', default='DINO+Point_MAE+Fusion', type=str,
                        choices=['DINO', 'Point_MAE', 'Fusion', 'DINO+Point_MAE', 'DINO+Point_MAE+Fusion',
                                 'DINO+Point_MAE+add'],
    ...
    parser.add_argument('--dataset_type', default='mvtec3d', type=str, choices=['mvtec_3d_processed', 'eyecandies'],
                        help='Dataset type for training or testing')
    parser.add_argument('--dataset_path', default='datasets/mvtec_3d_processed', type=str,
    ...
    parser.add_argument('--noise_std', default=0.05, type=float, help='noise_std')
    parser.add_argument('--aed_meta_epochs', default=1, type=int, help='aed_meta_epochs')
    parser.add_argument('--dsc_margin', default=0.8, type=float, help='dsc_margin')
